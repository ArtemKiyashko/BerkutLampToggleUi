...
const signalrendpoint = 'signalr';
const lampSetStateEndpoint = 'berkutlamp/setstate';
const lampGetStateEndpoint = 'berkutlamp/getstate';
...
function App() {
  ...
  const loadLampStatus = async () => {
    await fetch(`${apiBaseUrl}/${lampGetStateEndpoint}`)
      .then((response) => {
        return response.json();
      })
      .then((lampState) =>{
        setLampStatus(lampState.state);
      })
      .finally(() => {
        ...
        connection.on('lampstatuschanged', (lampStatus) => {
          setLampStatus(lampStatus.Status);
        });
  ...
  return (
    <Box sx={{
      ...
      alignItems: 'center',
      height: '100vh',
      backgroundImage: `${lampStatus ? `url(${BgImage})` : `url(${BgImageDark})`}`,
      backgroundSize: 'contain'
    }}>
      <MaterialUISwitch checked={lampStatus} onChange={handleIconClick} />
